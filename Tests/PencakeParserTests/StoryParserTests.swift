...
class StoryParserTests: XCTestCase {
    ...
    func testParsingFromURL() throws {
        ...
        let story = try storyParser.parse(directoryURL: storyURL, options: parseOptions)
        XCTAssertEqual(story, Constants.story)
    }
    
    func testParsingFromZipFile() throws {
        ...
        do {
            ...
            let parseOptions = ParseOptions(language: .english)
            let story = try storyParser.parse(zipFileURL: zipFileURL, options: parseOptions)
